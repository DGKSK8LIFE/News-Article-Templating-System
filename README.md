...
- Users are able to search through all created articles (uses an sql `LIKE` statement for now, then if I want better search results and database scanning: elastisearch) (shows users article preview which hyperlinks them to the article as a search result)
